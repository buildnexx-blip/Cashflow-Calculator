    ...
    <div className="min-h-screen p-4 md:p-8 max-w-7xl mx-auto relative">
      <header className="mb-8 flex flex-col md:flex-row items-center justify-between gap-4">
        <div>
          ...
          <h2 className="text-xl md:text-2xl font-serif font-bold text-[#064E2C] mt-1">Capital & Rental Growth</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <section className="bg-white rounded-[24px] p-6 shadow-sm border border-[#C6A672]/30">
             ...
             <InputGroup label="Stamp Duty" subLabel="(Auto or Edit)"><NumberInput prefix="$" value={inputs.stampDuty} onChange={(e) => handleInputChange('stampDuty', Number(e.target.value))} /></InputGroup>
          ...
          <section className="bg-white rounded-[24px] p-6 shadow-sm border border-[#C6A672]/30">
            <h2 className="text-xl font-serif font-semibold text-[#064E2C] mb-4">Assumptions</h2>
            <div className="grid grid-cols-2 gap-4">
          ...
          <div className="flex gap-4">
             ...
             <button onClick={() => setIsModalOpen(true)} className="flex-[2] py-4 rounded-full text-xs font-bold bg-[#064E2C] text-white hover:bg-[#053d22] transition-all shadow-xl shadow-green-900/20">Email Full PDF</button>
          </div>
        </div>
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <div className="flex justify-between items-center bg-[#064E2C]/5 p-4 rounded-2xl border border-[#064E2C]/10">
            <div className="flex items-center gap-2">
               ...
               <span className="text-xs font-bold text-[#064E2C] uppercase tracking-widest">Growth Perspective</span>
            </div>
            ...
            <div className="overflow-x-auto">
               <table className="w-full text-sm text-left">
                  ...
                  <tbody>
                    ...
                    <TableRow label="Solicitor & Due Diligence" value={inputs.solicitorFee + inputs.buildingPestFee} />
                    <tr className="font-bold border-t border-gray-200"><td className="px-4 py-4 text-[#064E2C]">Total Out of Pocket</td><td className="px-4 py-4 text-right text-[#064E2C]">${totalCashRequired.toLocaleString()}</td></tr>
                  </tbody>
               </table>
            </div>
          </div>
            ...
            <div className="overflow-x-auto">
               <table className="w-full text-sm text-left leading-relaxed">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    ...
                    <tr className="border-b border-gray-50 text-gray-500 italic"><td className="px-4 py-3 pl-8">Less: Vacancy Allowance</td><td className="px-4 py-3 text-right">-${Math.round(results.firstYearCashflow.vacancyLoss / 12).toLocaleString()}</td><td className="px-4 py-3 text-right">-${Math.round(results.firstYearCashflow.vacancyLoss).toLocaleString()}</td></tr>
                    <tr className="bg-[#FFFCED] font-semibold text-[#064E2C]"><td className="px-4 py-3">Effective Gross Rent</td><td className="px-4 py-3 text-right">${Math.round(results.firstYearCashflow.effectiveGrossRent / 12).toLocaleString()}</td><td className="px-4 py-3 text-right">${Math.round(results.firstYearCashflow.effectiveGrossRent).toLocaleString()}</td></tr>
                    <tr><td className="px-4 py-3 font-bold text-gray-900 bg-gray-50 uppercase tracking-widest text-[10px]" colSpan={3}>Operating Cash Expenses</td></tr>
                    <tr className="border-b border-gray-50"><td className="px-4 py-3 text-gray-700">Mortgage Debt Service</td><td className="px-4 py-3 text-right text-gray-500">${Math.round(results.firstYearCashflow.mortgageRepayments / 12).toLocaleString()}</td><td className="px-4 py-3 text-right font-medium">${Math.round(results.firstYearCashflow.mortgageRepayments).toLocaleString()}</td></tr>
                    <tr className="border-b border-gray-50"><td className="px-4 py-3 text-gray-700">Holding Costs (Rates/Insurance/Repairs)</td><td className="px-4 py-3 text-right text-gray-500">${Math.round(results.firstYearCashflow.otherOperatingExpenses / 12).toLocaleString()}</td><td className="px-4 py-3 text-right font-medium">${Math.round(results.firstYearCashflow.otherOperatingExpenses).toLocaleString()}</td></tr>
                    <tr className="border-t-2 border-gray-200 bg-gray-50 font-bold"><td className="px-4 py-4 text-gray-900">Net Cashflow (Pre-Tax)</td><td className={`px-4 py-4 text-right ${results.firstYearCashflow.netCashflow > 0 ? 'text-[#064E2C]' : 'text-red-500'}`}>${Math.round(results.firstYearCashflow.netCashflow / 12).toLocaleString()}</td><td className={`px-4 py-4 text-right ${results.firstYearCashflow.netCashflow > 0 ? 'text-[#064E2C]' : 'text-red-500'}`}>${Math.round(results.firstYearCashflow.netCashflow).toLocaleString()}</td></tr>
                    <tr><td className="px-4 py-3 font-bold text-gray-900 bg-gray-50 uppercase tracking-widest text-[10px]" colSpan={3}>Tax Strategy Benefits</td></tr>
                    <tr className="border-b border-gray-50 text-[#C6A672] italic"><td className="px-4 py-3">Depreciation (Paper Loss)</td><td className="px-4 py-3 text-right">${Math.round(results.firstYearCashflow.depreciation / 12).toLocaleString()}</td><td className="px-4 py-3 text-right font-medium">${Math.round(results.firstYearCashflow.depreciation).toLocaleString()}</td></tr>
                    <tr className="bg-[#064E2C]/5 font-bold text-[#064E2C]"><td className="px-4 py-3">Estimated ATO Refund</td><td className="px-4 py-3 text-right">${Math.round(results.firstYearCashflow.taxRefund / 12).toLocaleString()}</td><td className="px-4 py-3 text-right">${Math.round(results.firstYearCashflow.taxRefund).toLocaleString()}</td></tr>
                    <tr className="bg-[#064E2C] text-white font-bold"><td className="px-4 py-4 text-lg">Total Post-Tax Position</td><td className="px-4 py-4 text-right text-lg">${Math.round(results.firstYearCashflow.afterTaxCashflow / 12).toLocaleString()}</td><td className="px-4 py-4 text-right text-lg">${Math.round(results.firstYearCashflow.afterTaxCashflow).toLocaleString()}</td></tr>
                  </tbody>
               </table>
            </div>
          </div>
  ...
  <div className="bg-white">
      <div id="print-page-1" className="w-[800px] h-[1132px] bg-white p-16 font-sans text-gray-800 flex flex-col relative overflow-hidden">
         ...
         <div className="grid grid-cols-2 gap-12 mb-12">
            ...
            <div><h3 className="text-[10px] font-bold text-[#C6A672] uppercase tracking-[0.2em] mb-4">Acquisition Profile</h3><div className="bg-[#FFFCED]/30 p-6 rounded-3xl border border-[#C6A672]/10"><p className="text-xl font-serif font-bold text-[#064E2C]">{inputs.propertyAddress || 'Confidential Address'}</p><p className="text-sm text-gray-500 mt-2">{inputs.state} • Total Price: ${inputs.purchasePrice.toLocaleString()}</p></div></div>
         </div>
         <div className="bg-[#064E2C] p-10 rounded-[40px] shadow-2xl mb-12 relative overflow-hidden text-white"><h3 className="text-[11px] font-bold text-white/50 uppercase tracking-[0.3em] mb-8 text-center">First Year Performance Summary</h3><div className="grid grid-cols-5 gap-4"><div className="bg-white/5 rounded-2xl p-5 border border-white/10 text-center"><p className="text-[8px] font-bold text-white/40 uppercase tracking-widest mb-2">Gross Yield</p><p className="text-2xl font-serif font-bold">{((results.firstYearCashflow.effectiveGrossRent / inputs.purchasePrice) * 100).toFixed(2)}%</p></div><div className="bg-white/5 rounded-2xl p-5 border border-white/10 text-center"><p className="text-[8px] font-bold text-white/40 uppercase tracking-widest mb-2">Post-Tax (Wk)</p><p className="text-2xl font-serif font-bold">${Math.round(results.firstYearCashflow.afterTaxCashflow / 52).toLocaleString()}</p></div><div className="bg-white/5 rounded-2xl p-5 border border-white/10 text-center"><p className="text-[8px] font-bold text-white/40 uppercase tracking-widest mb-2">Pre-Tax (Wk)</p><p className="text-2xl font-serif font-bold">${Math.round(results.firstYearCashflow.netCashflow / 52).toLocaleString()}</p></div><div className="bg-white/5 rounded-2xl p-5 border border-white/10 text-center"><p className="text-[8px] font-bold text-white/40 uppercase tracking-widest mb-2">Tax Refund</p><p className="text-2xl font-serif font-bold">${Math.round(results.firstYearCashflow.taxRefund).toLocaleString()}</p></div><div className="bg-white/5 rounded-2xl p-5 border border-white/10 text-center"><p className="text-[8px] font-bold text-white/40 uppercase tracking-widest mb-2">Total Cash</p><p className="text-2xl font-serif font-bold">${(results.upfrontCostsTotal + inputs.purchasePrice * (inputs.depositPercent / 100)).toLocaleString()}</p></div></div></div>
         <div className="mt-auto pt-10 text-[9px] text-gray-300 italic border-t border-gray-100 flex justify-between uppercase tracking-widest"><div>© Homez Buyers Advocacy • Private Portfolio Analysis</div><div>Report 1 of 2</div></div>
      </div>
