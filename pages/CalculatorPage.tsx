...
const CalculatorPage: React.FC = () => {
  ...
  const generatePDF = async (userDetails: UserDetails): Promise<boolean> => {
    ...
    if (page1Element && page2Element && page3Element) {
      try {
        ...
        // Page 1
        const canvasOptions = { scale: 2, backgroundColor: '#ffffff', useCORS: true } as Parameters<typeof html2canvas>[1];
        ...
        // Page 3
        pdf.addPage();
        const canvas3 = await html2canvas(page3Element, canvasOptions);
  ...
  return (
    <div className="min-h-screen bg-[#FFFCED] p-4 md:p-8 lg:p-12 max-w-[1440px] mx-auto relative flex flex-col antialiased">
      <header className="mb-12 flex flex-col md:flex-row items-center justify-between gap-6 border-b border-[#C6A672]/20 pb-10 relative z-20">
        <div>
        ...
        <div className="flex flex-wrap items-center justify-center md:justify-end gap-3 md:gap-4 no-print w-full md:w-auto relative z-30">
          {/* Highly Visible Comparison Button */}
          <Link
            to="/compare"
            className="px-6 py-4 md:py-5 rounded-full font-black text-[#064E2C] bg-white border-2 border-[#064E2C] hover:bg-[#064E2C] hover:text-white transition-all text-xs md:text-sm uppercase tracking-widest whitespace-nowrap shadow-md hover:shadow-xl flex items-center gap-2"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" /></svg>
            Compare Scenarios
          </Link>

          {/* Status hidden on smaller screens to prioritize buttons */}
          ...
          <button onClick={() => setIsModalOpen(true)} className="px-8 md:px-10 py-4 md:py-5 rounded-full font-bold bg-[#064E2C] text-white shadow-2xl hover:bg-[#053d22] transition-all transform hover:-translate-y-1 active:scale-95 text-xs md:text-sm uppercase tracking-widest whitespace-nowrap">
            Download Report
          </button>
        </div>
      </header>
